...
//       formattedWorkingTime = parseFloat(
...
// This function now correctly calculates and formats time for the email.
exports.triggerEmailForEmployeeTimeSheet = async () => {
  try {
    const today = dayjs().format("YYYY-MM-DD");
    ...
    // Get all timesheets for today and populate employee
    const timesheets = await Timesheet.find({ date: today }).populate(
      "employeeId"
    );
    if (!timesheets || timesheets.length === 0) {
    ...
    for (const empName of sortedEmployees) {
      ...
      const totalTodayMinutes = empTimesheets.reduce(
        (sum, t) => sum + (t.workingTime || 0),
        0
      );

      const totalHours = formatMinutesToHHMM(totalTodayMinutes);
        ...
        .map(
          (t, idx) => `
                        <tr>
                            ...
                            <td>${t.task || "-"}</td>
                            <td>${dayjs(t.createdAt).format(
                              "DD-MM-YYYY HH:mm"
                            )}</td>
                            <td>${dayjs(t.updatedAt).format(
                              "DD-MM-YYYY HH:mm"
                            )}</td>
                            <td>${formatMinutesToHHMM(t.previousWork)}</td>
                            <td>${formatMinutesToHHMM(t.workingTime)}</td>
                            <td>${formatMinutesToHHMM(t.totalWork)}</td>
                        </tr>`
        )
        .join("");
