...
// ✅ Fully Refactored and Fixed User.jsx (MUI v6, Clean Logic, No Warnings)
import React, { useEffect, useState } from "react";
import {
  Container,
  Typography,
...
export default function User() {
  ...
  const handleSave = async () => {
    ...
    if (
      !form.name ||
      !form.email ||
      (editingIndex === null && !form.password) ||
      !form.role
    )
      return;
    ...
    try {
      ...
      const userPayload = {
        ...
        departmentId: finalDept._id,
        updatedBy: userId,
        updatedTime: new Date(),
      };

      if (editingIndex !== null) {
        await updateUser(users[editingIndex]._id, userPayload);
      } else {
        userPayload.createdBy = userId;
        userPayload.createdTime = new Date();
  ...
  const confirmDelete = async () => {
    try {
      await deleteUser(users[deleteIndex]._id);
      const updatedUsers = await getAllUsers();
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.aoa_to_sheet([
      ["Name", "Email", "Mobile", "Department", "Role"],
      ...users.map(({ name, email, mobile, departmentId, role }) => [
        name,
        email,
        mobile,
        departments.find((d) => d._id === departmentId)?.department || "N/A",
        role,
      ]),
    ]);
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    autoTable(doc, {
      head: [["Name", "Email", "Mobile", "Department", "Role"]],
      body: users.map(({ name, email, mobile, departmentId, role }) => [
        name,
        email,
        mobile,
        departments.find((d) => d._id === departmentId)?.department || "N/A",
  ...
  const filteredUsers = users.filter(({ name, email, mobile, role }) =>
    [name, email, mobile, role].some((val) =>
  ...
  return (
    <Container maxWidth="xl" sx={{ px: { xs: 2, md: 5 }, mt: 3, mb: 4 }}>
      ...
      <Grid container spacing={2} alignItems="center" my={2} columns={12}>
        {" "}
        {/* 💡 Use Grid for flexible layout of buttons and search */}
        <Grid columnSpan={{ xs: 12, sm: 6, md: 4 }}>
          {" "}
          {/* Buttons */}
          <Stack
            direction={isExtraSmallScreen ? "column" : "row"} // Stack vertically on extra small screens
                ...
                {[
                  ...
                  (head, i) => (
                    <TableCell
                      ...
                      onClick={head === "User" ? sortByName : undefined}
                    >
                      <Box display="flex" alignItems="center" gap={0.5}>
                        {head}
                        {head === "User" &&
        ...
        <DialogContent dividers>
          ...
          <Stack spacing={2} mt={1}>
            ...
            <TextField
              ...
              onChange={handleChange}
              fullWidth
              required={editingIndex === null} // Required only when adding a new user
              type="password"
              size="small"
            />
            <FormControl fullWidth size="small" required>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Are you sure you want to delete{" "}
            <strong>
              {deleteIndex !== null ? users[deleteIndex].name : ""}
            </strong>
            ? This action cannot be undone.
          </Typography>
        </DialogContent>
